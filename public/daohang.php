...
// ----------------------
// 开一个api的统一URL
function gdk_daohang_html_flush_rules(){
    $rules = get_option( 'rewrite_rules' );
    if ( !isset( $rules['^daohang(.*?)\.html$'] ) ) {
        global $wp_rewrite;
...
// 添加自定义URL重写
function gdk_daohang_html_custom_rewrite_rule() {
    add_rewrite_rule('^daohang(.*?)\.html$','index.php?daohang=gdkk$matches[1]','top');
}

function gdk_daohang_html_insert_query_vars( $vars ){
    array_push($vars, 'daohang');
    return $vars;
}

function gdk_daohang_html_cancel_redirect( $redirect_url ) {
	$api_type = get_query_var('daohang');
	if ( !empty($api_type) ){
		return false;
	}else{
		return $redirect_url; 
...
add_action( 'init', 'gdk_daohang_html_custom_rewrite_rule' , 10, 0);
add_filter( 'query_vars', 'gdk_daohang_html_insert_query_vars' );
add_filter( 'redirect_canonical', 'gdk_daohang_html_cancel_redirect' );
add_action( 'wp_loaded', 'gdk_daohang_html_flush_rules' );
...
<body>
    ...
    <header class="mobile-header-wrap">
        <a class="mobile-logo" href="/"><img src="http://chuangzaoshi.com/assets/images/logo_mobile.svg"></a>
    </header>
    ...
    <div class="sidenav">
        <a class="logo" href="/">
            <img src="http://chuangzaoshi.com/assets/images/logo.svg" alt="logo">
        </a>
        ...
        <?php
            ...
            echo '<ul class="nav-tags">';
                ...
                echo '</ul>';
                echo '<a class="copyright" href="about">&#169; 创造狮</a>
                </div>
                <div class="main-wrap">
                    <div class="main">';
        ...
        ?>
            ...
            <footer class="footer">
                ...
                Copyright &#169; 2016-2019 创造狮 创意工作者导航 <a href="http://www.miitbeian.gov.cn/"
                    style="color:#bbb;text-decoration: none;">赣ICP备14004527号-2</a>
